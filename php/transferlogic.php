...
if (isset($_POST["send"])) {
 ...
 //send money from sender to receiver
    $sql = "SELECT * FROM customer WHERE customer_id = '$receiver_acc'";
    $result = mysqli_query($db, $sql);
    //fetching receiver account details
    $row = mysqli_fetch_assoc($result);
//    die(var_dump($row));
    $receiverFn = $row["f_name"];
    $receiverLn = $row["l_name"];
    $receiver_id = $row["customer_id"];
    $receiver_cvc = $row["cvc"];
    $receiver_bal = $row["acct_bal"];
    ...
    if ($cvcgiven !== $cvc) {
        ...
        if ($sender_bal < $amount_sending) {
            ...
            if ($receiver_acc === $sender_acc) {
                ...
                if ($sender_bal >= 0) {
                    $sql = "UPDATE customer SET acct_bal = '$sender_bal' WHERE customer_id = '$sender_acc'";
                    $result = mysqli_query($db, $sql);
                    if (!$result) {
                        mysqli_rollback($db);
                    }
                    ...
                    $sql = "UPDATE customer SET acct_bal = '$receiver_bal' WHERE customer_id = '$receiver_acc'";
                    $result = mysqli_query($db, $sql);
                    if (!$result) {
                        mysqli_rollback($db);
                    }
                    ...
                    $sql = "INSERT INTO transactions (sender_id, receiver_id, amount, time) VALUES ('$sender_acc', '$receiver_acc', '$amount_sending', '$date_sent')";
                    $result = mysqli_query($db, $sql);
                    if (!$result) {
                        mysqli_rollback($db);
                    }
