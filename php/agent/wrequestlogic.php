...
if (isset($_POST['withdraw'])) {
    ...
    {
        $getBalance = "SELECT * FROM customer WHERE customer_id = '$givenAccNum'";
        $getBalResult = mysqli_query($db, $getBalance);
        $getBalRow = mysqli_fetch_assoc($getBalResult);
        $wBalance = $getBalRow['acct_bal'];
        ...
        else 
        {
            ...
            $sql = "UPDATE customer SET acct_bal = '$wBalance' WHERE customer_id = '$givenAccNum'";
            $result = mysqli_query($db, $sql);

            // inserting into transaction table
            $date_sent = date("Y-m-d H:i:s");
            $agent_id = "agent".$_SESSION['id']."";
            $sql = "INSERT INTO transactions (sender_id, receiver_id, amount, time) VALUES ('$agent_id', '$givenAccNum', '$wAmount', '$date_sent')";
            $result = mysqli_query($db, $sql);
